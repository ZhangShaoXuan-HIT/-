...
极化类型	振幅条件	相位差 δ = φ_y - φ_x	旋向（沿 + z 观察）
线极化	任意	0° 或 180°	无旋转
右旋圆极化	E_x0 = E_y0	-90°	顺时针
左旋圆极化	E_x0 = E_y0	+90°	逆时针
右旋椭圆极化	振幅不等	δ < 0	顺时针
左旋椭圆极化	振幅不等	δ > 0	逆时针
功能特点：
...
3.将上面「二、完整 Python 代码」全部复制粘贴到这个文件中
4.按 Ctrl + S 保存文件
...
└── PolarizationWaveSimulator 类  # 主仿真器类
    ...
    └── run()                     # 启动仿真
核心物理公式：
•E_x(z,t) = E_x0 · cos(ωt - kz)
•E_y(z,t) = E_y0 · cos(ωt - kz + δ)
旋向判断（沿 +z 方向观察）：
•δ < 0 → 电场顺时针旋转 → 右旋
•δ > 0 → 电场逆时针旋转 → 左旋
